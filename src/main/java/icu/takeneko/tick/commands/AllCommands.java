...
import icu.takeneko.tick.helpers.Messenger;
...
public class AllCommands {
    ...
    public static final LiteralArgumentBuilder<ICommandSender> COMMAND_TICK = literal(COMMAND_TICK_NAME).
        then(
            literal("rate").
                then(
                    argument("value", FloatArgumentType.floatArg(0.01f)).
                        executes(context -> {
                            float arg = context.getArgument("value", Float.class);
                            Messenger.m(context.getSource(), "w Current tps is ", Messenger.resolve(String.format("wb %.1f", arg), null));
        ...
        then(
            literal("warp").
                then(
                    argument("value", LongArgumentType.longArg(1)).
                        executes(context -> {
                            long amount = context.getArgument("value", Long.class);
                            EntityPlayer player = context.getSource() instanceof EntityPlayer ? (EntityPlayer) context.getSource() : null;
